...
*Setup instructions for a Gitian build of Diabase Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Diabase
...
- [Building Diabase Core](#building-diabase-core)
...
Clone the git repositories for Diabase Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/diabasecoin/diabase
...
in the Diabase Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Diabase Core
----------------

To build Diabase Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Diabase Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/diabase/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/diabasecoin/diabase
...
For example:
```bash
URL=https://github.com/crowning-/diabase.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit diabase=${COMMIT} --url diabase=${URL} ../diabase/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit diabase=${COMMIT} --url diabase=${URL} ../diabase/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit diabase=${COMMIT} --url diabase=${URL} ../diabase/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the diabase git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../diabase/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/diabase
SIGPATH=/some/root/path/diabase-detached-sigs

./bin/gbuild --url diabase=${BTCPATH},signature=${SIGPATH} ../diabase/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/diabase-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/diabase-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/diabase-osx-build.assert
```
...
[diabase/gitian.sigs](https://github.com/diabasecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
