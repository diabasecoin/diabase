Cross-compiliation of Diabase Core
===============================

Diabase Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Diabase.
...
When building Diabase Core, use
...
It's not possible to build the Diabase Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Diabase Core source code).
