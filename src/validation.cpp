                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Diabase
...
//        nSubsidyBase = (11111.0 / (pow((dDiff+51.0)/6.0,2.0)));
 //       if(nSubsidyBase > 500) nSubsidyBase = 5;
 //       else if(nSubsidyBase < 2.5) nSubsidyBase = 2.5;
//    } else {
        // GPU/ASIC mining era
        // 2222222/(((x+2600)/9)^2)
//        nSubsidyBase = (2222222.0 / (pow((dDiff+2600.0)/9.0,2.0)));
//        if(nSubsidyBase > 2.5) nSubsidyBase = 2.5;
...
static int64_t nTimeDiabaseSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(DIAC): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // DIAC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(DIAC): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(DIAC): spork is off, skipping transaction locking checks\n");
    }
    ...
    // DIAC : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(DIAC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(DIAC): couldn't find masternode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeDiabaseSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Diabase specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeDiabaseSpecific * MICRO, nTimeDiabaseSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(DIAC): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
