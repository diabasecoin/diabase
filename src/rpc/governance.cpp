...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 DIAC irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on diabasecentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in diabase.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("diabase.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in diabase.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Diabase features */
    { "diabase",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "diabase",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "diabase",               "gobject",                &gobject,                {} },
    { "diabase",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
