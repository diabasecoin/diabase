...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s diabase-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  diabase-tx [options] <hex-tx> [commands]  Update hex-encoded diabase transaction\n" +
              "  diabase-tx [options] -create [commands]   Create hex-encoded diabase transaction\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded diabase transaction
